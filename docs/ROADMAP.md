...
#### 7. 画像/動画アップロード
- **対応**: Supabase Storage にファイル保存（画像のみ実装済）
- **UI**: アップロードボタン、プレビュー、プログレスバー
- **DB**: `posts` テーブルに `image_url` カラム追加済み
- **残タスク**: 動画アップロード、複数画像対応
...
#### 25. テストフレームワーク
- **対応**: Playwright for E2Eテスト（実装済）
- **対象**: 認証、投稿、いいね、フォロー、検索、プロフィール機能
- **残タスク**: コンポーネント単体テスト（Jest + React Testing Library）
